...
## Can we map? - produces same output as above
...
map variants ... returns vector, collection of numbers

```{r}
map_dbl(list_norm, median)

map_df(list_norm, mean_and_sd) # collapse each into tables

ouput = map_df(list_norm, mean_and_sd) # save output
```
...
This really is a list column!

```{r}
weather_nest_df[["data"]]
```

Regression of tmax on tmin; get different regression coefficients each time because I'm building a simple linear regression on a different data set on each of these lines of code.
...
Write a short lil ol function to write a linear model for all 3 things
...
Here's my scraping function that work for a single page
...
napoleon_reviews %>% 
  select(-page_url) %>% 
  unnest(reviews) #get rid of list of tibbles
```
